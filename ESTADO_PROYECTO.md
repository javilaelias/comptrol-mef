...
| Intangibles: pestaña "Sin condición: datos SIGA" | 🟡 **Desplegado en staging 2026-09-24** (rama `feat/intangibles-sin-condicion-siga`, pusheada; `main` sin tocar); falta que el usuario confirme en pantalla para archivar | `openspec/changes/intangibles-sin-condicion-datos-siga/` (concilio 5 revisores, APROBADO CON CAMBIOS). 932 bienes sin condición: 916 con fin de vida útil SIGA, 548 OC verificadas por ítem, 35 no verificadas (N° de orden de SIGA apunta a otra orden, ej. `140400030005`), 349 por NEA. Respaldos: `/tmp/comptrol-antes-sin-condicion-20260924-121229.dump` (contenedor) + copia en `D:\MV\Comptrol-MEF-respaldos\`, código en `apps/{api,web}.bak.20260924-121229`. Licencias (196), alertas (4) y activos (37,369) sin cambios tras recargar SIGA. Ajuste mismo día (`30f25c5`, desplegado): columna Situación = No perpetuo (fin de vida vencido, 570) / Vigente (346) / Perpetua (sin fecha, 16); la verificación de la orden pasó a su propia columna. Segundo ajuste (`28853b3`, desplegado): clic en un bien → su orden reconstruida desde SIGA (cabecera, proveedor, ítems, totales; `po_detail` JSONB, migración `20260924130000`); 48 órdenes, 9,272 bienes; SIGA no guarda el PDF firmado. Respaldo previo: `staging-comptrol-antes-po-detail-20260924-123528.dump` + `apps/{api,web}.bak.20260924-123528`. `cerrar-app.ps1` quedó sin correr (el usuario lo cortó) |
