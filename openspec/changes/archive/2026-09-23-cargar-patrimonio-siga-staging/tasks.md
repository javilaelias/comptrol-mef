...
- [x] 1.1 Confirmar con el usuario el momento para recrear el contenedor `db` en staging (breve
...
- [x] 1.2 Editar `docker-compose.server.yml` (repo local): agregar al servicio `db` el bloque
      `ports: ["127.0.0.1:5437:5432"]` (nunca sin el prefijo `127.0.0.1:`). Commitear y
...
- [x] 1.3 En el servidor (SSH), con `-f` explícito siempre (ese directorio no tiene
      ...
      '{{.State.Health.Status}}'` → `healthy`) y que el puerto quedó atado a `127.0.0.1:5437`,
      no a `0.0.0.0` (`docker port comptrol-postgres` debe mostrar `127.0.0.1:5437`).
- [x] 1.4 Verificar que `comptrol-api` se recuperó de la recreación de `db`: `curl
      http://localhost:3001/api/v1/health/ready` desde el servidor devuelve `200` — no asumir
      que Prisma reconectó solo.
- [x] 1.5 Abrir el túnel SSH local (`ssh -L 5437:127.0.0.1:5437 usr_admin@10.118.67.55`) y
      verificar conexión: `psql postgresql://postgres:postgres@localhost:5437/comptrol -c
      "select 1"` responde sin error.

## 2. Dry-run y verificación

- [x] 2.1 Correr `SIGA_DRY_RUN=1 npm run import:siga` con `DATABASE_URL` apuntando al túnel
      (`localhost:5437/comptrol`) y `SIGA_DATABASE_URL` sin cambios (local). Capturar el resumen
      completo (conteos + tabla de ejemplos).
- [x] 2.2 Comparar el resumen contra `docs/IMPORTACION_SIGA.md` (34,207 activos totales, 3,245
...
- [x] 2.3 Verificar por consulta directa (vía el mismo túnel) que el tenant `mef` que ve el
...
- [x] 3.1 Mostrar el resumen del dry-run (tarea 2.1) al usuario y pedir confirmación explícita
...
- [x] 3.2 Con confirmación: correr `SIGA_DRY_RUN=0 npm run import:siga` contra el mismo túnel.
      Verificar que el proceso termina con el resumen final (activos creados/actualizados) sin
      errores.
- [x] 3.3 Verificar en la app real: entrar a Comptrol-MEF (portal OPDA Apps, SSO) y confirmar que
      el dashboard "Activos" ya no muestra 0 — el total debe acercarse al del dry-run.
- [x] 3.4 Cerrar el túnel SSH. Dejar el puerto del servidor tal como quedó en la tarea 1.2 (atado
...
- [x] 4.1 Actualizar `docs/IMPORTACION_SIGA.md` si los conteos reales de staging difirieron de
...
- [x] 4.2 Crear `ESTADO_PROYECTO.md` conciso en la raíz de `D:\MV\Comptrol-MEF` (primer cambio
      formal de este repo en la fábrica) con el estado post-import.
- [x] 4.3 Commit de los archivos ya existentes en el working tree (`import-siga.ts`, cambios de
      ...
      sesión.
- [x] 4.4 Archivar el cambio (`openspec archive`) tras QA.
